...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an intelligent assistant that helps create work items from a user's voice command. The user can speak in English or Hinglish (a mix of Hindi and English).
Your goal is to parse the user's command and extract the necessary information to fill out a work item creation form.
...
Here are the available Work Items that can be dependencies:
{{#each availableTasks}}
- ID: {{id}}, Title: "{{title}}"
{{/each}}
